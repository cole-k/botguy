...
function helpDefault(args, session) {
    session.send(`Learn more about a particular command with _help command_\n
...
function helpMemeify(args, session) {
    session.send(`Memeify: Takes a string and increases the spaces between its characters successively\n
...
function helpDetonate(args, session) {
    session.send(`Detonate: Takes a string and 'detonates' it\n
...
function helpWhois(args, session) {
    session.send(`Whois: Takes a person and describes them\n
        Called like: whois person\n
        Supported people: botguy, here
        `);
}
function helpNiceOne(args, session) {
    session.send(`Niceone: Give or list nice ones\n
...
function helpYam(args, session) {
    session.send(`Yam: Retrieve or post to the meme machine (meme-machine.xyz)\n
